      //globals
        ...
        function executeMove(activePlayer, startingTriangleIdx, activeDieIdx){
            ...
            if(game.usedDice[0] === 1 && game.usedDice[1] === 1 && game.activePlayer === 0){
               startTurn()
            }
  ...
  /*          ctx.fillStyle = 'white';
      ...
      function autoMove() {
          ...
          let options = []
          //adds all possible moves to options list.
          for (let i = 0; i < 2; i++) {
              for (let j = 0; j < 26; j++) {
          ...
          if(options.length > 0){
              let bestGame = [evaluateBoard(options[0][0].board,options[0][0].activePlayer),options[0]]
              for(let i=0; i<options.length;i++){
                  let boardScore = evaluateBoard(options[i][0].board,options[i][0].activePlayer)
                  if(boardScore > bestGame[0]){
                      bestGame = [evaluateBoard(options[i][0].board,options[i][0].activePlayer),options[i]]
                  }
              }
              game = deepCopy(oGame);
              setTimeout(function () {
        ...
        function evaluateBoard(board,player){
            ...
            //console.log(board)
            //console.log("board score before"+board[player*25].amount)
            //boardScore += board[0].amount*10;

            boardScore += board[player*25].amount*2
            //console.log("board score "+boardScore)
            return boardScore
        }

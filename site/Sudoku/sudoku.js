    ...
    function create_board() {
        //propagates the table that represents the board and sets it up
    ...
    function turn_button_to_input(id) {
        //squares turn from buttons to input fields when a user clicks them. This handles that
    ...
    function turn_input_to_button() {
        //input fields are turned back into buttons when the user selcts a different button.
    ...
    function find_collisions(space_id, value, board){
        //checks if the given value for the given square is a legal move
    ...
    //determines if the given value can go in the given section of the board
        //section meaning ninth of the board
    ...
    function solve_puzzle(){
        //solves the game and then visualizes the backtracking algorithm
        playThrough = true
        log = []
        select_button(-1)
    ...
    function play_log(solve_log,index = 0){
        //this is kind of a mess - it visualizes the backtracking algorithm on the current board.
        //backtracking steps stored in solve_log
        let play_speed = 1000/(document.getElementById("speed_slider").value)//this is the speed it solves the board, decided by the slider
        if(document.getElementById("speed_slider").value === 100){
            play_speed = 0
        }
        if(index < solve_log.length && playThrough){ //checks if the solve_log is finished
        ...
        }else{//when the log is finished the speed slider is hidden again
            document.getElementById("speed_slider").style.opacity = '0'
            select_button(-1)
        }
    }
    function style_button(id,backgroundColor,color){
        //sets the style of a given button with specified colors
    ...
    function solve(board,index = 0){
        //solves the game using backtracking more information here https://en.wikipedia.org/wiki/Sudoku_solving_algorithms
    ...
    function generate_board() {
        //generates a new random board with a unique solution
    ...
    function prune_board(board) {
        //removes values from the board until no more can be removed while th board still has a unique solution
        let index_list = []
        for(let i=0; i<81; i++){
            index_list.push(i)
        }
    ...
    function set_seed() {
        //generates the seed for a new board -- a 9x9 board with one of each number 1-9
        let seed = []
        for(let i=0;i<9;i++) {
            seed.push(i + 1)
            for (let j = 0; j < 8; j++) {
